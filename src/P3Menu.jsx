...
export default function P3Menu({ onNavigate, sfxMuted = true }) {
	const [active, setActive] = useState(0);
	const [mounted, setMounted] = useState(false);
	const [animKey, setAnimKey] = useState(0);
	const { playHover, playConfirm } = usePersonaSfx({ muted: sfxMuted });
			...
			<div className="p3-overlay">
				...
				<div className={`p3-hint ${mounted ? "mounted" : ""}`}>
					<div className="p3-hint-row">
						<span className="p3-hint-key">MOUSE</span>
					...
					<div className="p3-hint-row">
						<span className="p3-hint-key">ENTER/SPACE</span>
						<span>CONFIRM</span>
					</div>
				</div>
			</div>
